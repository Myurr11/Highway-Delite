...
app.get('/api/experiences/:id', async (req: Request, res: Response) => {
  try {
    ...
    const result = await pool.query(
      'SELECT * FROM experiences WHERE id = $1 AND active = true',
      [id]
    );
...
app.get('/api/availability/:experienceId', async (req: Request, res: Response) => {
  try {
...
app.post('/api/bookings',
  [
    ...
    body('finalTotal').isNumeric().withMessage('Total amount is required')
  ],
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      await client.query(
        ...
        [refId, experienceId, slotId, fullName, email, quantity, finalTotal, promoCode || null, 'confirmed']
      );
...
app.get('/api/bookings/:refId', async (req: Request, res: Response) => {
  try {
